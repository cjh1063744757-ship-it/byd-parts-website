...
models = [
  {
    'id': 'E2', 'name': 'BYD E2',
    'info': {'year':'2019-2024','type':'Compact EV Hatchback','voltage':'60V Li-Fe','motor':'90kW'},
    'parts': [
  ...
  {
    'id': 'Atto3', 'name': 'BYD Atto 3',
    'info': {'year':'2021-2024','type':'Compact EV SUV','voltage':'400V Li-Fe / NMC','motor':'150kW'},
    'parts': [
  ...
  {
    'id': 'Seal', 'name': 'BYD Seal',
    'info': {'year':'2022-2024','type':'Mid-size EV Sedan','voltage':'800V CTB','motor':'AWD 390kW'},
    'parts': [
  ...
  {
    'id': 'Dolphin', 'name': 'BYD Dolphin',
    'info': {'year':'2021-2024','type':'Compact EV Hatchback','voltage':'400V Blade','motor':'95kW'},
    'parts': [
  ...
  {
    'id': 'HanEV', 'name': 'BYD Han EV',
    'info': {'year':'2020-2024','type':'Full-size EV Sedan','voltage':'400V Blade','motor':'AWD 480kW'},
    'parts': [
  ...
  {
    'id': 'TangEV', 'name': 'BYD Tang EV',
    'info': {'year':'2019-2024','type':'Full-size EV SUV 7-seater','voltage':'400V Blade','motor':'AWD 505kW'},
    'parts': [
  ...
  {
    'id': 'SongPlusEV', 'name': 'BYD Song Plus EV',
    'info': {'year':'2021-2024','type':'Mid-size EV SUV','voltage':'400V Blade','motor':'135kW'},
    'parts': [
  ...
  {
    'id': 'Seagull', 'name': 'BYD Seagull',
    'info': {'year':'2023-2024','type':'Mini EV Hatchback','voltage':'400V Blade','motor':'60kW / 75kW'},
    'parts': [
  ...
  {
    'id': 'YuanUP', 'name': 'BYD Yuan UP',
    'info': {'year':'2024-2025','type':'Compact EV SUV (Atto 2)','voltage':'400V Blade','motor':'FWD 150kW'},
    'parts': [
...
MODEL_FILL  = PatternFill('solid', start_color='2C5282')
MODEL_FONT  = Font(name='Arial', bold=True, color='FFFFFF', size=10)
PRICE_FONT  = Font(name='Arial', bold=True, color='1E8449', size=10)
...
CAT_FILLS = {
  'Body & Exterior':        PatternFill('solid', start_color='D6EAF8'),
  'Lighting':               PatternFill('solid', start_color='FEF9E7'),
  'Doors, Glass & Mirrors': PatternFill('solid', start_color='FDEBD0'),
  'Brakes':                 PatternFill('solid', start_color='FADBD8'),
  'Suspension & Steering':  PatternFill('solid', start_color='D5F5E3'),
  'EV Drivetrain':          PatternFill('solid', start_color='E8DAEF'),
  'Cooling System':         PatternFill('solid', start_color='D6EAF8'),
  'Interior':               PatternFill('solid', start_color='F2F3F4'),
  'Wheels & Tyres':         PatternFill('solid', start_color='FDEBD0'),
  'Maintenance':            PatternFill('solid', start_color='D1F2EB'),
}

def sc(ws, r, c, v, font=None, fill=None, align=None, fmt=None):
...
ws = wb.active
ws.title = 'BYD Parts - All Models'

# 大标题
ws.merge_cells('A1:I1')
c = ws['A1']
c.value = 'BYD OEM Spare Parts Catalog  |  CM IMPORT & EXPORT  |  FOB China Wholesale  |  WhatsApp: +86 173 1830 5222'
c.font = Font(name='Arial', bold=True, color='FFFFFF', size=12)
c.fill = PatternFill('solid', start_color='1B3A6B')
c.alignment = Alignment(horizontal='center', vertical='center')
ws.row_dimensions[1].height = 28

ws.merge_cells('A2:I2')
c = ws['A2']
c.value = 'All prices are FOB China reference prices in USD. Final pricing depends on order quantity. Contact us for confirmed quotation.'
c.font = Font(name='Arial', italic=True, color='5D4037', size=9)
c.fill = PatternFill('solid', start_color='FFF8E1')
c.alignment = Alignment(horizontal='center', vertical='center')
ws.row_dimensions[2].height = 18

# 列头
headers = ['#', 'Model', 'Category', 'Part Name (EN)', 'Part Name (CN)', 'Part Number', 'Unit Price (USD)', 'MOQ (pcs)', 'Condition / Notes']
widths  = [5,   16,      22,         38,               22,               18,            16,                  10,           30]
for i, (h, w) in enumerate(zip(headers, widths), 1):
    sc(ws, 3, i, h, font=HEADER_FONT, fill=HEADER_FILL, align=CENTER)
...
for m in models:
    ...
    c = ws[f'A{row}']
    info = m['info']
    c.value = f"  {m['name']}   |   {info['year']}   |   {info['type']}   |   Motor: {info['motor']}   |   {info['voltage']}"
    c.font = MODEL_FONT
    c.fill = MODEL_FILL
    c.alignment = LEFT
    c.border = BDR
    ...
    for p in m['parts']:
        ...
        note_val = (cond + '  ' + notes).strip() if notes else cond
        sc(ws, row, 1, num,      align=CENTER, fill=fill)
        sc(ws, row, 2, m['name'],              fill=fill)
        sc(ws, row, 3, cat,                    fill=fill)
        sc(ws, row, 4, name_en,                fill=fill)
        sc(ws, row, 5, name_cn,                fill=fill)
        sc(ws, row, 6, part_no,                fill=fill)
        sc(ws, row, 7, price,    font=PRICE_FONT, fill=fill, align=CENTER, fmt='"$"#,##0.00')
        sc(ws, row, 8, moq,      align=CENTER, fill=fill)
        sc(ws, row, 9, note_val,               fill=fill)
...
ws.merge_cells(f'A{row}:I{row}')
c = ws[f'A{row}']
c.value = f'TOTAL  —  {num-1} SKUs  |  {len(models)} Models  |  CM IMPORT & EXPORT'
c.font = Font(name='Arial', bold=True, color='FFFFFF', size=10)
c.fill = PatternFill('solid', start_color='1B3A6B')
c.alignment = Alignment(horizontal='center', vertical='center')
c.border = BDR

out = 'BYD_Parts_Website_Current.xlsx'
wb.save(out)
print(f'OK: {num-1} parts, {len(models)} models -> {out}')
